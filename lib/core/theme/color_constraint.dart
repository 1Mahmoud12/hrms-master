import 'package:flutter/material.dart';

class AppColors {
  //static Color primarycolor = Color(0xff635F54);
  static const white = Color(0xffffffff);
  static const black = Color(0xFF000000);
  static const amber = Color(0xffFF8F00);

  static Color red = Colors.red;
  static Color transparent = Colors.transparent;
  static Color grey = Colors.grey;
  static const green = Color(0xff12B76A);
  static const scaffoldBackGround = Color(0xffF0F2F8);
  static const primaryColor = Color(0xff0A256C);
  static const textColor = Color(0xff212121);
  static const subTextColor = Color(0xff616161);
  static const periodicMaintenanceColor = Color(0xffFBE2DC);
  static const malfunctionRequestColor = Color(0xffE2F5E5);
  static const emergencyRequestColor = Color(0xffFFEAA4);
  static const periodicMaintenanceIconColor = Color(0xffFFBEAC);
  static const malfunctionRequestIconColor = Color(0xff9FE5D0);
  static const emergencyRequestIconColor = Color(0xffFFDD6B);
  static const dividerColor = Color(0xffBAC8DC);
  static const selectedCharData = Color(0xff82CAE0);
  static const unSelectedCharData = Color(0xff292D30);
  static const fillColorTextFormField = Color(0xffF5F6FA);
  static const cFillColorDropDownButton = Color(0xffe5e4f4);
  static const textColorTextFormField = Color(0xff939FBE);
  static const greyWhite = Color(0xffF0F2F5);
  static const colorNumber = Color(0xff344054);
  static const colorNumberToday = Color(0xffF2F4F7);
  static const cBorderButton = Color(0xffD0D5DD);
  static const cBorderSVG = Color(0xffEEEEEE);
  static const hintOrange = Color(0xffFD7224);
  static const cPlusNumber = Color(0xff7F56D9);
  static const cLightPlusNumber = Color(0xffF9F5FF);
  static const cDateMaintenance = Color(0xff20affe);
  static const cBorderDecoration = Color(0xffEEEEEE);
  static const cBackGroundDateMaintenance = Color(0xffe8f6ff);
  static const cBackGroundIconMaintenance = Color(0xffD0DEFC);
  static const cBackGroundDeleteButton = Color(0xffED4141);
  static const cBackGroundConfirmButton = Color(0xff25BA69);
  static Color cBackGroundIconButton = const Color(0xff246BFD).withOpacity(.2);
}
